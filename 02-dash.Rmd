...
As with Shiny, in the rest of this chapter, we'll highlight the code used to make app, and the design choices behind the code.
...
def cols_header (data_records):
    ...
    col_names = data_records[0].keys()
    cols = list(
      map(lambda i: {'name': i, 'id': i}, col_names)
    )
...
I'm a sucker for functional programming, so I'm using a `map()` function.
...
```{r eval=FALSE}
purrr::map(col_names, ~list(name = .x, id = .x))
```
...
We would need to `import functools`, then use a reducer:

```{python eval=FALSE} 
def append(dictionary, key):
    dictionary[key] = func_agg
    return dictionary

dict_agg = functools.reduce(append, cols_agg, {})
```
...
You'll note another appearance of `list(map())` where we set the `options`.
