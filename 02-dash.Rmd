...
The first function we'll verify is used to tell us, for a given data frame (sent as records), and a given type, which columns are of that type.
...
Note that the first thing we do is reconstitute the Pandas data frame from the records.
Return a list of column names that satisfies the predicate in `select_types()`.
...
Note that we are being sent the records, rather than a the data frame. 
...
For convenience, we'll write this as in terms of data frames; we'll see later that we adapt this function to the callback, which uses records. 
