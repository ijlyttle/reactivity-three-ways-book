...
- `.every(predicate)`: return `true` if every value in the array satisfies the `predicate` function we supplies.
...
In arquero, we supply an object to describe rollup (aggregation) operations: 
...
Thus, we build the rollup object by using a `reduce()` function on the `cols_group` array:
...
values <- purrr::reduce(
  ...
  ~c(
    ...
    rlang::list2(
      "{.y}" := list(
        expr = glue::glue('(d) => op.{func_agg}(d["{.y}"])'), 
        func = TRUE
      )
    )
  ),
...
This gets a little heavy because we have to use `rlang::list2()` to interpolate the names: `"{.y}"`.
